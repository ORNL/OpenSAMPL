...
from opensampl.db.access_orm import APIAccessKey, Roles, UserRole, Users, Views, add_user_role
...
def test_views(db_session):
    """Test Views model."""
    view = Views(name="test_view", description="Test View", query="SELECT * FROM test_table", created_at=datetime.now())
    db_session.add(view)
    db_session.commit()
...
def test_roles(db_session):
    """Test Roles model."""
    role = Roles(name="test_role", description="Test Role", permissions=["read", "write"], created_at=datetime.now())
    db_session.add(role)
    db_session.commit()
...
def test_user_role(db_session):
    ...
    role = Roles(name="test_role", description="Test Role", permissions=["read"], created_at=datetime.now())
    ...
    user_role = db_session.query(UserRole).filter_by(user_id=user.id, role_id=role.id).first()
...
def test_view_query_validation(db_session):
    ...
    with pytest.raises(ValueError):
        Views(name="invalid_view", description="Invalid View", query="INVALID SQL QUERY", created_at=datetime.now())
