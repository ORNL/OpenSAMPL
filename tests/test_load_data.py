...
from opensampl.db.orm import Locations, TestMetadata, ProbeMetadata, ProbeData
...
def test_load_probe_metadata(mock_session, sample_probe_data):
    ...
    result = load_probe_metadata(
        VendorType.ADVA,
        ProbeKey(probe_id="TEST001", ip_address="192.168.1.1"),
        sample_probe_data,
...
def test_route_or_direct_decorator():
    ...
    with patch("opensampl.load_data.ENV_VARS.ROUTE_TO_BACKEND.get_value", return_value=True), \
    ...
    with patch("opensampl.load_data.ENV_VARS.ROUTE_TO_BACKEND.get_value", return_value=False), \
...
@pytest.mark.parametrize("test_input,expected", [
    ...
    (
        {
            "table": "probe_metadata",
            "data": pd.DataFrame({"invalid_col": [1]}),
            "conflict": "update"
        },
